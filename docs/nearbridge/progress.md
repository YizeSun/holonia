...
| Build Week P0/P1 | 完成 | 54/54 测试通过；macOS 与通用 iOS Device SDK 构建通过；Mac bundle 双 XPC 嵌入确认 | 未运行 | 单一真实 iPhone/Mac 设备对上的 readiness、相关联 execution receipt、acknowledgement 与净化导出主路径通过 | [`build-week-p0-p1-results.md`](build-week-p0-p1-results.md) |
...
Build Week P0/P1 不增加网络或 Agent 权限，也不占用已规划的 NB-10；它把纵向路径重组为评审默认 Demo 与详细 Diagnostics，增加下一步引导、示例问题、signed execution receipt、净化导出和 OpenAI privacy-preserving session safety identifier。共享测试已增至 54 项并通过，macOS 与通用 iOS Device SDK 构建成功，Mac bundle 嵌入两个 XPC。2026-07-21 的同一真实设备对证据显示两端 readiness 全部 ready、相同 invocation/capability 成功、iPhone acknowledgement sent、Mac received，且净化导出不含 prompt、answer 或凭据；这不升级延期错误、稳定性和多设备矩阵。
