...
Status: **Implementation and automated checkpoint passed; core pairing/authentication was later observed in the final integrated physical build, while restart/revocation validation remains pending.**
...
- trust-store 更新是事务式的：Keychain 写入失败时不发布“已配对”或“已撤销”，并停止继续建立信任。
...
2026-07-21 在最终集成 build 上观察到相同六位确认码、双端人工批准、`established`、paired records、成功 Keychain 写入与 `Authentication: authenticated`。没有执行关闭重启后的持久性、fresh-proof reconnect、revoke 或 re-pair 矩阵。详见 [`physical-validation-2026-07-21.md`](physical-validation-2026-07-21.md)。
