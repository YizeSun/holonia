...
**Core integrated physical path observed successfully; deferred physical matrix remains open.**

This run validates the NB-1 through NB-5 main vertical path in the final integrated build. It does not claim that each historical checkpoint commit was installed separately, nor that every stage-specific failure, restart, revocation, or boundary case was exercised.
