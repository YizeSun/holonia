...
项目当前处于概念固定与 NearBridge 第一阶段实现期，尚未冻结公开协议。NearBridge `NB-0` 的普通 Wi-Fi 真机核心路径已经跑通；`NB-1` 到 `NB-5` 的实现和自动化验证已经完成，各自的真机验证等待独立执行。当前是 feature-complete code checkpoint，不是生产就绪版本。
